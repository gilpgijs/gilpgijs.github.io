import { miFooter } from "./miFooter.js"
   ...
   for (let i = 0, len = anclas.length; i < len; i++) {
    ...
    if (href === ancla.href || href + "index.html" === ancla.href
     ...
     if (i === 0) {
      ...
      if (ancla.dataset.prev) {
       urlAnterior = ancla.dataset.prev
       contenidoAnterior = ancla.dataset.prevcont || ""
      } else {
       const anclaAnterior = anclas[i - 1]
       urlAnterior = anclaAnterior.href
       contenidoAnterior = anclaAnterior.textContent || ""
       if (contenidoAnterior === "▲" || contenidoAnterior === "▼") {
        contenidoAnterior = anclaAnterior.title
       }
      }
      for (const anterior of anteriores) {
       anterior.href = urlAnterior
     ...
     if (i >= len - 1) {
      ...
      if (contenidoSiguiente === "▲" || contenidoSiguiente === "▼") {
       contenidoSiguiente = anclaSiguiente.title
      }
      for (const siguiente of siguientes) {
       siguiente.href = urlSiguiente
